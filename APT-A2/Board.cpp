	...
	for(int i = 0; i < this->yEnd; i++){
		...
		for(int j = 0; j < this->xEnd; j++){
			...
			// using cout<<'colour'<<...<<'reset'; set the colour and reset it at the end of cout to return the colour back to normal
				...
				if(this->gameBoard[i][j]->getColour()=='G'){
				cout << green << gameBoard[i][j]->getColour()<<gameBoard[i][j]->getShape() << reset;
				}
				else if(this->gameBoard[i][j]->getColour()=='B'){
				cout << blue << gameBoard[i][j]->getColour()<<gameBoard[i][j]->getShape() << reset;
				}
				else if(this->gameBoard[i][j]->getColour()=='R'){
				cout << red << gameBoard[i][j]->getColour()<<gameBoard[i][j]->getShape() << reset;
				}
				else if(this->gameBoard[i][j]->getColour()=='Y'){
				cout << yellow << gameBoard[i][j]->getColour()<<gameBoard[i][j]->getShape() << reset;
				}
				else if(this->gameBoard[i][j]->getColour()=='P'){
				cout << purple << gameBoard[i][j]->getColour()<<gameBoard[i][j]->getShape() << reset;
				}
				else if(this->gameBoard[i][j]->getColour()=='O'){
				cout << orange << gameBoard[i][j]->getColour()<<gameBoard[i][j]->getShape() << reset;
				}
